...
type Form = { slug: string; firstName: string; lastName: string; role: string; title: string; qualifications: string; admissions: string; bio: string; email: string; phone: string; photoUrl: string; practiceAreas: string; sortOrder: number; isPublished: boolean };
const empty: Form = { slug: "", firstName: "", lastName: "", role: "Associate", title: "", qualifications: "", admissions: "", bio: "", email: "", phone: "", photoUrl: "", practiceAreas: "", sortOrder: 0, isPublished: true };
...
export default function AdminPeople() {
  ...
  const openEdit = (p: NonNullable<typeof people>[0]) => setModal({
    mode: "edit", id: p.id,
    form: { slug: p.slug, firstName: p.firstName, lastName: p.lastName, role: p.role, title: p.title ?? "", qualifications: p.qualifications ?? "", admissions: p.admissions ?? "", bio: p.bio ?? "", email: p.email ?? "", phone: p.phone ?? "", photoUrl: p.photoUrl ?? "", practiceAreas: (p.practiceAreas ?? []).join(", "), sortOrder: p.sortOrder, isPublished: p.isPublished }
  });

  const handleSave = () => {
    if (!modal) return;
    const areas = modal.form.practiceAreas.split(",").map(s => s.trim()).filter(Boolean);
    const data = { ...modal.form, practiceAreas: areas };
    if (modal.mode === "create") {
      create.mutate({ data: data as never }, { onSuccess: () => { invalidate(); setModal(null); toast({ title: "Person created" }); }, onError: () => toast({ title: "Error", variant: "destructive" }) });
    } else {
      update.mutate({ id: modal.id!, data: data as never }, { onSuccess: () => { invalidate(); setModal(null); toast({ title: "Person updated" }); }, onError: () => toast({ title: "Error", variant: "destructive" }) });
    }
  };

  const handleDelete = (id: number) => {
    if (!confirm("Delete this person?")) return;
    remove.mutate({ id }, { onSuccess: () => { invalidate(); toast({ title: "Person deleted" }); }, onError: () => toast({ title: "Error", variant: "destructive" }) });
  };
            ...
            {(people ?? []).map(p => (
              <div key={p.id} data-testid={`row-person-${p.id}`} className="flex items-center justify-between bg-[#151515] border border-[#2A2A2A] px-6 py-4">
                <div>
                  ...
                  <p className="text-[#B8B8B8] text-xs mt-1">{p.role} · {p.isPublished ? <span className="text-green-400">Published</span> : <span className="text-red-400">Draft</span>}</p>
                </div>
          ...
          <div className="bg-[#151515] border border-[#2A2A2A] p-8 w-full max-w-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={modal.form.role} onChange={e => setModal(m => m && ({ ...m, form: { ...m.form, role: e.target.value } }))}
              ...
              {fields.map(({ label, key, ta, num }) => (
                <div key={key}>
                  ...
                  {ta ? (
                    ...
                    <input type={num ? "number" : "text"} value={(modal.form as never)[key]} onChange={e => setModal(m => m && ({ ...m, form: { ...m.form, [key]: num ? Number(e.target.value) : e.target.value } }))}
                      className="w-full bg-[#0E0E0E] border border-[#2A2A2A] text-[#F7F4EE] px-3 py-2 text-sm focus:border-[#C6A15B] focus:outline-none" />
                  )}
                </div>
              ))}
              <div className="flex items-center gap-3">
                <input type="checkbox" id="published" checked={modal.form.isPublished} onChange={e => setModal(m => m && ({ ...m, form: { ...m.form, isPublished: e.target.checked } }))} className="w-4 h-4 accent-[#C6A15B]" />
